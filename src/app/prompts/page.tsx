    ...
    <div className="bg-background">
        ...
        <div className="container mx-auto px-8 text-center relative z-10">
          <div className="max-w-4xl mx-auto">
            <div className="flex justify-center mb-5">
              <Lightbulb className="w-10 h-10 text-primary-foreground/80 stroke-[2.5]" />
            </div>
            ...
            <p className="mt-8 text-xl md:text-2xl text-primary-foreground/95 max-w-3xl mx-auto font-medium">
              Pick a Theme, Spark an Idea, Share Your Genius!
            </p>
          </div>
        </div>
      </div>
      ...
      <section className='relative'>
        ...
        <div className="relative z-10 container mx-auto px-4 sm:px-8 py-20">
            {/* Current Theme Spotlight */}
            <div className="mb-20">
            <h2 className="text-3xl md:text-4xl font-bold text-center text-foreground font-heading mb-10">
                This Month's Theme
            </h2>
            <Card className="grid md:grid-cols-2 items-center gap-8 p-6 md:p-8 bg-card text-card-foreground border-[3px] border-foreground shadow-[12px_12px_0px_0px_hsl(var(--foreground))] hover:shadow-[14px_14px_0px_0px_hsl(var(--foreground))] transition-shadow duration-200">
                <div className="rounded-lg overflow-hidden border-[3px] border-foreground shadow-[6px_6px_0px_0px_hsl(var(--foreground))]">
                {currentThemeImage && <Image
                    src={currentThemeImage.src}
                    alt={currentThemeImage.alt}
                    width={currentThemeImage.width}
                ...
                <div>
                <CardHeader className="p-0 mb-4">
                    <span className="text-sm font-bold uppercase tracking-wider bg-secondary text-secondary-foreground px-3 py-1.5 rounded-full w-fit mb-3 border-[2px] border-foreground shadow-[3px_3px_0px_0px_hsl(var(--foreground))]">
                    {currentTheme.tag}
                    </span>
                    <CardTitle className="text-3xl md:text-4xl font-bold font-heading text-primary">
                    {currentTheme.title}
                    </CardTitle>
                </CardHeader>
                <CardContent className="p-0">
                    ...
                    <Button asChild size="lg" className="text-lg font-bold bg-accent hover:bg-accent/90 text-accent-foreground border-[3px] border-foreground shadow-[5px_5px_0px_0px_hsl(var(--foreground))] hover:shadow-[7px_7px_0px_0px_hsl(var(--foreground))] hover:-translate-y-0.5 transition-all duration-200">
                    <Link href="/submissions/new">
                        Submit for This Theme <ArrowRight className="ml-2" />
                    </Link>
                    </Button>
                </CardContent>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {otherThemes.map((prompt, index) => {
                const image = placeholderImages.find(p => p.hint.includes(prompt.hint.toLowerCase()));
                const rotations = ['-rotate-1', 'rotate-1'];
                const rotationClass = rotations[index % 2];
                
                return (
                    <Card key={prompt.title} className={`flex flex-col group overflow-hidden border-[3px] border-foreground shadow-[6px_6px_0px_0px_hsl(var(--foreground))] hover:shadow-[10px_10px_0px_0px_hsl(var(--primary))] hover:border-primary transition-all duration-300 hover:-translate-y-1.5 ${rotationClass} hover:rotate-0`}>
                    <CardHeader className="p-0">
                        <div className="relative h-48 w-full">
                        {image ? <Image
                            src={image.src}
                        ...
                        /> : <div className="bg-muted w-full h-full"></div>}
                        <div className="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition-colors"></div>
                        <span className="absolute top-4 left-4 text-xs font-bold uppercase tracking-wider bg-card text-card-foreground px-3 py-1.5 rounded-full border-[2px] border-foreground shadow-[3px_3px_0px_0px_hsl(var(--foreground))]">
                            {prompt.tag}
                        </span>
                        </div>
                    </CardHeader>
                    <CardContent className="p-6 flex-grow flex flex-col">
                        <CardTitle className="text-xl leading-tight font-heading text-foreground mb-2 group-hover:text-primary transition-colors">
                        {prompt.title}
                        </CardTitle>
                        <p className="text-muted-foreground text-sm mb-4 flex-grow leading-relaxed">
                        {prompt.description}
                        </p>
                        <Button variant="outline" className="self-start mt-auto border-[3px] border-foreground shadow-[4px_4px_0px_0px_hsl(var(--foreground))] hover:shadow-[6px_6px_0px_0px_hsl(var(--foreground))] hover:-translate-y-0.5 transition-all duration-200" asChild>
                        <Link href="/submissions/new">Explore & Submit</Link>
                        </Button>
                    </CardContent>
                    </Card>
            ...
            <div className="text-center max-w-3xl mx-auto mb-12">
                ...
                Choose a category that best fits your amazing project!
                </p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                {submissionCategories.map((category, index) => {
                ...
                return (
                    <Link href="/submissions" key={category.title}>
                    <Card className={`group flex flex-col items-center text-center gap-4 p-6 h-full border-[3px] border-foreground shadow-[6px_6px_0px_0px_hsl(var(--foreground))] hover:shadow-[10px_10px_0px_0px_hsl(var(--primary))] hover:border-primary transition-all duration-300 hover:-translate-y-1.5 ${rotationClass} hover:rotate-0`}>
                        ...
                        <category.icon className="w-12 h-12 text-secondary stroke-[2.5]" />
                        </div>
                        <h3 className="font-bold text-xl text-foreground font-heading">
                        {category.title}
                        </h3>
            ...
            <div className="bg-gradient-to-r from-secondary to-primary p-10 rounded-xl border-[3px] border-foreground shadow-[10px_10px_0px_0px_hsl(var(--foreground))] text-primary-foreground rotate-1 hover:rotate-0 hover:shadow-[12px_12px_0px_0px_hsl(var(--foreground))] transition-all duration-300">
                ...
                Don't wait for a prompt! If you have a brilliant project, we want to see it. Share your genius with us today!
                </p>
